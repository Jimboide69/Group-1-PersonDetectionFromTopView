...
class SSDLossMetrics:
    ...
    def iou_m(self, y_true, y_pred):
        ...
        Arguments:
            y_true (nD tensor): A TensorFlow tensor of any shape containing the ground truth data.
                In this context, the expected tensor contains the ground truth bounding box 
                coordinates, where the last dimension contains `(xmin, xmax, ymin, ymax)`.
        ...
        return (K.sum(inter) + K.epsilon()) / (K.sum(union) + K.epsilon())
    ...
    def f1_m(self, y_true, y_pred):
        ...
        Returns:
            The F1 score, a nD-1 Tensorflow tensor. In this context a 2D tensor
    ...
    def compute_loss(self, y_true, y_pred):
        ...
        classification_loss = tf.to_float(self.f1_m(y_true[:,:,:-12], y_pred[:,:,:-12])) # Output shape: (batch_size, n_boxes)
        localization_loss = tf.to_float(self.iou_m(y_true[:,:,-12:-8], y_pred[:,:,-12:-8])) # Output shape: (batch_size, n_boxes)
