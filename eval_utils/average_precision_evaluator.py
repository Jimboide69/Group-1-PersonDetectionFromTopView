...
class Evaluator:
    '''
    Computes the mean average precision and IOU of the given Keras SSD model on the given dataset.
    ...
    def __call__(self,
        ...
        # Compile the returns.
        if return_average_matches or return_precisions or return_recalls or return_average_precisions:
            ret = [mean_average_precision]
            if return_average_matches:
                ret.append(self.average_matches)
    ...
    def match_predictions(self,
        ...
        for class_id in range(1, self.n_classes + 1):
            ...
            for i in tr:
                ...
                gt_match_overlaps.append(gt_match_overlap)
        ...
        self.gt_match_overlaps = gt_match_overlaps

        if ret:
            return true_positives, false_positives, cumulative_true_positives, cumulative_false_positives, gt_match_overlaps 
    ...
    def compute_average_precisions(self, mode='sample', num_recall_points=11, verbose=True, ret=False):
        ...
        if (self.gt_match_overlaps is None):
            raise ValueError("IOU not available. You must run `match_predictions()` before you call this method.")
        ...
        for class_id in range(1, self.n_classes + 1):
            ...
            if verbose:
                ...
                print("Computing average IOU, class {}/{}".format(class_id, self.n_classes))

            cumulative_precision = self.cumulative_precisions[class_id]
            cumulative_recall = self.cumulative_recalls[class_id]
            gt_match_overlap = self.gt_match_overlaps[class_id]
            ...
            average_match /= gt_match_overlap.size

            average_matches.append(average_match)
            average_precisions.append(average_precision)

        self.average_matches = average_matches
        self.average_precisions = average_precisions

        if ret:
            return average_precisions, average_matches
